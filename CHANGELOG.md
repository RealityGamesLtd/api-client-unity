...
- **NDJSON streams now allow gzip** (EXPERIMENT). `CreatePutStreamRequest` sets `AllowCompressedResponse = true`, routing the request through a third `HttpClient` whose handler decompresses — Mono derives `Accept-Encoding` from the *handler's* `AutomaticDecompression` at send time, which is why streams were excluded when gzip landed for regular requests (#11). JSON gzips ~8–10x, which proportionally cuts Mono's TLS record-buffer churn. SSE stays uncompressed on purpose: a proxy that buffers gzip output would hold its messages back. **Restoration point:** delete the `AllowCompressedResponse` line in `ApiClientConnection.CreatePutStreamRequest` to restore uncompressed streams exactly. On device, verify cells still stream progressively during a wave rather than arriving batched at response end.
...
- `StreamReader` is constructed with an explicit 64 KB read buffer (it previously took the 1024-byte default). Every refill is one `SslStream.ReadAsync`, for which Mono allocates a fresh ~16 KB `BufferOffsetSize2` per operation — a ~16x amplification of every byte of stream traffic, and the single largest allocator in the app at 29.7 MB over 2440 reads (22% of all bytes) in the 2026-08-05 capture. Explicitly *not* addressed: that TLS record buffer scales with wire bytes, not read size (measured unchanged going from 1 KB to 16 KB refills) — hence the gzip experiment above.
